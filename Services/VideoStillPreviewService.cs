...
public static class VideoStillPreviewService
{
    ...
    public static string? ResolveFfmpegExecutable(string? configuredVideoToolDirectory)
    {
        return VideoToolResolutionService.Resolve(configuredVideoToolDirectory).FfmpegPath;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!string.Equals(Path.GetFileName(ffmpegPath), "ffmpeg.exe", StringComparison.OrdinalIgnoreCase))
        {
            return new VideoStillPreviewResult
            {
                Success = false,
                ErrorMessage = "動画定点プレビュー抽出に失敗しました（設定されたツールが ffmpeg.exe ではありません）。"
            };
        }
