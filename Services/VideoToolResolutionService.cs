...
{
    public static VideoToolResolutionResult Resolve(string? configuredVideoToolDirectory)
    {
        string? trimmedPath = string.IsNullOrWhiteSpace(configuredVideoToolDirectory)
            ? null
            : configuredVideoToolDirectory.Trim();

        bool IsTargetExe(string path, string exeName)
        {
        ...
        foreach ((string candidate, string source) in EnumerateFfmpegCandidates(trimmedPath))
        {
            if (File.Exists(candidate) && IsTargetExe(candidate, "ffmpeg.exe"))
            {
        ...
        foreach ((string candidate, string source) in EnumerateFfplayCandidates(trimmedPath, ffmpegPath))
        {
            ...
            if (ffplayPath is null && File.Exists(candidate) && IsTargetExe(candidate, "ffplay.exe"))
            {
        ...
        seen.Clear();
        foreach ((string candidate, string source) in EnumerateFfprobeCandidates(trimmedPath, ffmpegPath))
        {
            if (string.IsNullOrWhiteSpace(candidate) || !seen.Add(candidate))
            {
                continue;
            ...
            if (ffprobePath is null && File.Exists(candidate) && IsTargetExe(candidate, "ffprobe.exe"))
            {
    ...
    private static IEnumerable<(string Candidate, string Source)> EnumerateFfmpegCandidates(string? configuredPath)
    {
        if (!string.IsNullOrWhiteSpace(configuredPath))
        {
            ...
            {
                if (string.Equals(Path.GetFileName(configuredPath), "ffmpeg.exe", StringComparison.OrdinalIgnoreCase))
                {
                ...
                else
                {
                    ...
                    {
                        yield return (Path.Combine(dir, "ffmpeg.exe"), "指定ファイル親フォルダ");
                        yield return (Path.Combine(dir, "bin", "ffmpeg.exe"), "指定ファイル親フォルダ/bin");
                    }
                }
            }
        ...
        if (!string.IsNullOrWhiteSpace(configuredPath))
        {
            if (File.Exists(configuredPath))
            {
                ...
                else
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(dir))
                    {
                        yield return (Path.Combine(dir, "ffplay.exe"), "指定ファイル親フォルダ");
                        yield return (Path.Combine(dir, "bin", "ffplay.exe"), "指定ファイル親フォルダ/bin");
                    }
                }
            }
            else if (Directory.Exists(configuredPath))
    ...
    private static IEnumerable<(string Candidate, string Source)> EnumerateFfprobeCandidates(string? configuredPath, string? resolvedFfmpegPath)
    {
        ...
        if (!string.IsNullOrWhiteSpace(configuredPath))
        {
            if (File.Exists(configuredPath))
            {
                if (string.Equals(Path.GetFileName(configuredPath), "ffprobe.exe", StringComparison.OrdinalIgnoreCase))
                {
                    yield return (configuredPath, "指定パス");
                }
                else
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(dir))
                    {
                        yield return (Path.Combine(dir, "ffprobe.exe"), "指定ファイル親フォルダ");
                        yield return (Path.Combine(dir, "bin", "ffprobe.exe"), "指定ファイル親フォルダ/bin");
                    }
                }
            }
            else if (Directory.Exists(configuredPath))
            {
                ...
                yield return (Path.Combine(configuredPath, "ffmpeg", "bin", "ffprobe.exe"), "指定フォルダ/ffmpeg/bin");
            }
        }
